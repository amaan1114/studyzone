      ...
      <div className="relative mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
        {step === 0 && (
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="mx-auto mb-5 flex size-14 items-center justify-center rounded-2xl bg-[#0e7de8]/10">
              <MessageSquareDashed className="size-7 text-[#0e7de8]" />
            </div>
            <p className="text-xs font-semibold uppercase tracking-[0.32em] text-[#0e7de8]">We're Listening</p>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 1 && !anonymous && (
                <motion.div key="name" {...slideIn}>
                  ...
                  <h3 className={questionCls}>What's your name? <span className="text-[#0e7de8]">*</span></h3>
              ...
              {step === 2 && anonymous && (
                <motion.div key="email-skip" {...slideIn}>
                  <p className={labelCls}>Question 2</p>
                  <h3 className={questionCls}>Which study center are you giving feedback for? <span className="text-[#0e7de8]">*</span></h3>
                    ...
                    {LOCATIONS.map((loc, i) => (
                      <button key={loc} type="button" onClick={() => { setLocation(loc); setTimeout(next, 280); }}
                        className={`flex w-full items-center gap-3 rounded-2xl border px-5 py-3.5 text-left text-sm font-medium transition-all hover:-translate-y-0.5 ${
                          location === loc ? "border-[#0e7de8] bg-[#0e7de8]/6 text-[#0e7de8]"
                            : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:border-[#0e7de8]/50"
                        }`}>
                        <span className="flex size-6 shrink-0 items-center justify-center rounded-full border border-current text-[10px] font-bold opacity-60">
              ...
              {step === 3 && !anonymous && (
                <motion.div key="location" {...slideIn}>
                  ...
                  <h3 className={questionCls}>Which study center? <span className="text-[#0e7de8]">*</span></h3>
                  <div className="mt-5 space-y-3">
                    {LOCATIONS.map((loc, i) => (
                      <button key={loc} type="button" onClick={() => { setLocation(loc); setTimeout(next, 280); }}
                        className={`flex w-full items-center gap-3 rounded-2xl border px-5 py-3.5 text-left text-sm font-medium transition-all hover:-translate-y-0.5 ${
              ...
              {step === 3 && anonymous && (
                <motion.div key="type-anon" {...slideIn}>
                  <p className={labelCls}>Question 3</p>
                  <h3 className={questionCls}>What type of feedback is this? <span className="text-[#0e7de8]">*</span></h3>
                    ...
                    {FEEDBACK_TYPES.map((t, i) => (
                      <button key={t} type="button" onClick={() => { setFeedbackType(t); setTimeout(next, 280); }}
                        className={`flex items-center gap-2 rounded-2xl border px-4 py-3 text-sm font-medium transition-all hover:-translate-y-0.5 ${
                          feedbackType === t ? "border-[#0e7de8] bg-[#0e7de8]/6 text-[#0e7de8]"
                            : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:border-[#0e7de8]/50"
                        }`}>
              ...
              {step === 4 && !anonymous && (
                <motion.div key="type" {...slideIn}>
                  ...
                  <h3 className={questionCls}>What type of feedback? <span className="text-[#0e7de8]">*</span></h3>
                  <div className="mt-5 grid grid-cols-2 gap-3">
                    {FEEDBACK_TYPES.map((t, i) => (
                      <button key={t} type="button" onClick={() => { setFeedbackType(t); setTimeout(next, 280); }}
                        className={`flex items-center gap-2 rounded-2xl border px-4 py-3 text-sm font-medium transition-all hover:-translate-y-0.5 ${
              ...
              {step === 4 && anonymous && (
                <motion.div key="msg-anon" {...slideIn}>
                  <p className={labelCls}>Question 4</p>
                  <h3 className={questionCls}>Your message <span className="text-[#0e7de8]">*</span></h3>
              ...
              {step === 5 && anonymous && (
                <motion.div key="submit-anon" {...slideIn} className="text-center">
                  ...
                  <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">
                    Submitting anonymous <span className="font-semibold text-[#0e7de8]">{feedbackType}</span> for <span className="font-semibold text-[#0e7de8]">{location}</span>.
                  </p>
              ...
              {step === 6 && !anonymous && (
                <motion.div key="submit" {...slideIn} className="text-center">
                  ...
                  <h3 className="mt-2 text-2xl font-bold text-[#201657] dark:text-white sm:text-3xl">Ready to submit?</h3>
                  <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">
                    Submitting as <span className="font-semibold text-[#201657] dark:text-white">{name}</span> — <span className="font-semibold text-[#0e7de8]">{feedbackType}</span> for <span className="font-semibold text-[#0e7de8]">{location}</span>.
                  </p>
                  <button onClick={() => setSubmitted(true)}
                    className="mt-8 inline-flex cursor-pointer items-center gap-2 rounded-full bg-[#0e7de8] px-10 py-3.5 text-sm font-semibold text-white shadow-[0_12px_30px_rgba(14,125,232,0.25)] transition-all hover:-translate-y-0.5 hover:bg-[#0a6fd4] active:scale-[0.98]">
                    <Send className="size-4" />Submit Feedback
