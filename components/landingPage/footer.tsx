...
export function Footer() {
  return (
    <footer className="relative overflow-hidden bg-[#f8fafc] dark:bg-slate-800 text-[#201657] dark:text-white">
      ...
      <div className="absolute -right-24 top-0 h-64 w-64 rounded-full bg-[#0e7de8] opacity-10 blur-3xl" />
              ...
              <ul className="mt-5 space-y-3 text-slate-600 dark:text-slate-400">
                {group.links.map((link) => (
                  <li key={link.label}>
                    <MotionLink
                      href={link.href}
                      className="inline-flex cursor-pointer items-center text-sm transition-colors duration-200 hover:text-[#0e7de8]"
                      whileHover={{ x: 4 }}
                      whileTap={{ scale: 0.98 }}
          ...
          <motion.div variants={itemVariants}>
            ...
            <div className="mt-5 space-y-4 text-sm text-slate-600 dark:text-slate-400">
              <div className="flex items-start gap-3">
                <MapPin className="mt-0.5 h-5 w-5 shrink-0 text-[#0e7de8]" />
                <p>
                  Delhi and Bangalore study centers across prime learning locations.
                </p>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="h-5 w-5 shrink-0 text-[#0e7de8]" />
                <motion.a href="tel:+919355623363" className="cursor-pointer transition-colors duration-200 hover:text-[#0e7de8]" whileHover={{ x: 4 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.2, ease: "easeOut" as const }}>
                  +91 93556 23363
                </motion.a>
              </div>
              <div className="flex items-center gap-3">
                <Mail className="h-5 w-5 shrink-0 text-[#0e7de8]" />
                <motion.a href="mailto:studyzone365.com@gmail.com" className="cursor-pointer transition-colors duration-200 hover:text-[#0e7de8]" whileHover={{ x: 4 }} whileTap={{ scale: 0.98 }} transition={{ duration: 0.2, ease: "easeOut" as const }}>
