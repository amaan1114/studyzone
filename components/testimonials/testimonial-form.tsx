...
import { Star, Upload, ArrowRight, Check } from "lucide-react";
      ...
      <div className="relative mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
        {step === 0 && (
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
            ...
            <p className="text-xs font-semibold uppercase tracking-[0.32em] text-[#0e7de8]">Your Voice Matters</p>
        ...
        <div className="overflow-hidden rounded-[2rem] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-[0_14px_40px_rgba(15,23,42,0.07)]">
          <div className="h-1 w-full bg-slate-100 dark:bg-slate-700">
            <motion.div className="h-full bg-[#0e7de8]" animate={{ width: `${progress}%` }}
            ...
            <AnimatePresence mode="wait">
              {step === 0 && (
                <motion.div key="name" {...slideIn}>
                  ...
                  <h3 className={questionCls}>What's your full name? <span className="text-[#0e7de8]">*</span></h3>
              ...
              {step === 1 && (
                <motion.div key="email" {...slideIn}>
                  ...
                  <h3 className={questionCls}>Your email address? <span className="text-[#0e7de8]">*</span></h3>
              ...
              {step === 2 && (
                <motion.div key="occupation" {...slideIn}>
                  ...
                  <h3 className={questionCls}>Your occupation or field of study? <span className="text-[#0e7de8]">*</span></h3>
              ...
              {step === 3 && (
                <motion.div key="center" {...slideIn}>
                  ...
                  <h3 className={questionCls}>Which study center did you visit? <span className="text-[#0e7de8]">*</span></h3>
              ...
              {step === 4 && (
                <motion.div key="testimonial" {...slideIn}>
                  ...
                  <h3 className={questionCls}>Share your experience in your own words. <span className="text-[#0e7de8]">*</span></h3>
              ...
              {step === 5 && (
                <motion.div key="rating" {...slideIn}>
                  ...
                  <h3 className={questionCls}>How would you rate your experience? <span className="text-[#0e7de8]">*</span></h3>
                    ...
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button key={s} type="button" onClick={() => { setRating(s); setTimeout(next, 300); }}
                        ...
                        <Star className={`size-10 transition-colors ${
                          s <= (hovered || rating) ? "fill-[#0e7de8] text-[#0e7de8]" : "fill-slate-100 dark:fill-slate-700 text-slate-300"}`} />
                      </button>
                    ))}
                  </div>
                ...
                <motion.div key="photo" {...slideIn}>
                  ...
                  <label className="mt-6 flex cursor-pointer items-start gap-3">
                    <input type="checkbox" checked={consent} onChange={e => setConsent(e.target.checked)}
                      className="mt-1 size-4 rounded border-slate-300 accent-[#0e7de8]" />
              ...
              {step === 7 && (
                <motion.div key="submit" {...slideIn} className="text-center">
                  ...
                  <p className="mt-4 text-slate-500 dark:text-slate-400 text-sm">Submitting as <span className="font-semibold text-[#201657] dark:text-white">{name}</span> for <span className="font-semibold text-[#0e7de8]">{center}</span>.</p>
                  <button
                    onClick={() => setSubmitted(true)}
                    className="mt-8 inline-flex cursor-pointer items-center justify-center gap-2 rounded-full bg-[#0e7de8] px-10 py-3.5 text-sm font-semibold text-white shadow-[0_12px_30px_rgba(14,125,232,0.25)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#0a6fd4] active:scale-[0.98]">
                    Submit Testimonial
                  </button>
                </motion.div>
              )}
        ...
        {step > 0 && !submitted && (
          <button onClick={() => setStep(s => s - 1)}
            className="mt-4 text-xs text-slate-400 hover:text-[#0e7de8] transition-colors">
            ← Back
          </button>
        )}
      </div>
